...
export const StorageService = {
  ...
  getActiveAuthors: async (): Promise<User[]> => {
    // This function is now public-safe. It derives authors from public stories.
  ...
  getStoriesByAuthor: async (authorId: string): Promise<Story[]> => {
    // FIX: Perform two separate, secure queries that the rules can validate.
    const draftsQuery = query(collection(db, "stories"), where("authorId", "==", authorId), where("status", "==", "draft"));
    const publishedQuery = query(collection(db, "stories"), where("authorId", "==", authorId), where("status", "==", "published"));
    ...
    const drafts = draftsSnapshot.docs.map(d => ({ id: d.id, ...d.data() } as Story));
    const published = publishedSnapshot.docs.map(d => ({ id: d.id, ...d.data() } as Story));
  ...
  initiateAdminActionCode: async(userId: string): Promise<string> => {
    const code = Math.floor(1000 + Math.random() * 9000).toString();
    const userDocRef = doc(db, 'users', userId);
    await updateDoc(userDocRef, { actionConfirmationCode: code });
    return code;
  },

  deleteStoryWithVerification: async (userId: string, code: string, storyId: string): Promise<boolean> => {
    const userDoc = await getDoc(doc(db, 'users', userId));
    if (userDoc.exists() && userDoc.data().actionConfirmationCode === code) {
      await deleteDoc(doc(db, 'stories', storyId));
      await updateDoc(doc(db, 'users', userId), { actionConfirmationCode: null }); // Invalidate code
      return true;
    }
    return false;
  },
