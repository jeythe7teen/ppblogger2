...
export const AllCompletedNovelsPage: React.FC = () => {
    const { t } = useTranslations();
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const completedStories = allPublishedStories.filter(s => s.classification === 'completed');
            
            const activeAuthors = await StorageService.getActiveAuthors();
            setAuthors(activeAuthors);
            
            const storiesWithStats: StoryWithStats[] = completedStories.map(story => {
    ...
    return (
        <div className="max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <h1 className="text-2xl sm:text-3xl lg:text-4xl font-heading font-bold text-white">{t('allCompletedNovels.title')}</h1>
            ...
            {filteredStories.length > 0 ? (
                ...
                <p className="text-gray-400 text-center py-12">{t('allCompletedNovels.noNovels')}</p>
            )}
        </div>
    );
};